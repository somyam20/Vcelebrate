...
class ModelConfig:
    ...
    async def initialize_db_pool(self):
        ...
        try:
            db_host = os.getenv("LLM_DB_HOST")
            db_port = os.getenv("LLM_DB_PORT")
            db_name = os.getenv("LLM_DB_NAME")
            db_user = os.getenv("LLM_DB_USER")
            db_password = os.getenv("LLM_DB_PASSWORD")
